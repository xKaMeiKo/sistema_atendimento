...
if cargo in ['Atendente', 'Manutenção', 'Financeiro']:
    ...
    with col_direita:
        ...
        if st.session_state.view_modo == "Aguardando":
            ...
            # Vinculamos o 'value' dos campos ao nosso apagador automático do state
            c1, c2 = st.columns(2)
            ...
            if st.button("Registrar Chamado", type="primary", use_container_width=True):
                if pessoa and solicitacao_detalhe:
                    ...
                    # Esvaziamos as variáveis de texto logo após gravar no banco de dados
                    st.session_state.form_pessoa = ""
                    st.session_state.form_solicitacao = ""
                    ...
                    st.rerun() # Reinicia a página aplicando as caixas em branco
                else:
...
# --- FLUXO 3: VISÃO DO SUPERVISOR ---
    ...
    if not df.empty:
        ...
        st.info("Nenhum dado encontrado no banco.")import streamlit as st
import pandas as pd
from supabase import create_client, Client
from datetime import datetime, timezone
import time
