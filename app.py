...
# --- BARRA LATERAL FIXA (Não quebra o Streamlit) ---
with st.sidebar:
...
# --- BLOCOS DO PAINEL PRINCIPAL COM REFRESH SEPARADO ---
if cargo in ['Atendente', 'Manutenção', 'Financeiro']:
    
    # Criamos o fragmento apenas para a área de conteúdo e listagem interna, evitando tocar na barra lateral global
    @st.fragment(run_every=15)
    def renderizar_conteudo_com_refresh():
        
        # Criamos uma seção para os chamados ativos dentro do corpo do site, ou simulada na lateral
        col_esquerda, col_direita = st.columns([1, 3])
        
        with col_esquerda:
            sub_c1, sub_c2 = st.columns([3, 1])
            sub_c1.subheader("📌 Chamados")
            if sub_c2.button("🔄", help="Forçar atualização agora"):
                st.rerun()
        ...
        with col_direita:
            ...
            if st.session_state.view_modo == "Aguardando":import streamlit as st
import pandas as pd
from supabase import create_client, Client
from datetime import datetime
import time
...
# --- DECORADOR DE FRAGMENTO OFICIAL ---
# Envelopamos o conteúdo em uma função fragmentada para atualizar a cada 15 segundos nativamente
@st.fragment(run_every=15)
def renderizar_painel_operacional():
    ...
    if cargo in ['Atendente', 'Manutenção', 'Financeiro']:
        if st.session_state.view_modo == "Aguardando":
            st.info("💡 Selecione um chamado na barra lateral para tratar ou clique em '➕ Novo Atendimento'. O painel se atualiza automaticamente a cada 15 segundos.")
            ...
            if tid:
                ...
                st.info("Selecione um chamado na barra lateral.")

# --- EXECUÇÃO BASEADA NO CARGO ---
if cargo in ['Atendente', 'Manutenção', 'Financeiro']:
